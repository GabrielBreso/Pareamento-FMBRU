        ...
        <main>
            ...
            <script>
                ...
                function salvarResultadosPDF(resultados, tabela1, tabela2) {
                    ...
                    const naoPareadosTabela1 = tabela1.filter(p => !resultados.some(r => r.pessoaTabela1 === p['Nome completo'])).map(p => [p['Nome completo']]);
                    const naoPareadosTabela2 = tabela2.filter(p => !resultados.some(r => r.pessoaTabela2 === p['Nome completo'])).map(p => [p['Nome completo']]);
